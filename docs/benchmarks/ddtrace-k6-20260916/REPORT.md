...
1. **本报告已用 10 分钟正式窗口替换原 90 秒结果。** 100/300 RPS 共 30 个窗口均用于持续资源和 GC 判断；500 RPS 因短测已出现约 24.5% 超时，不进入长测容量结论。
2. **300 RPS 三类全开相对无探针：** 平均延迟 34.77→37.86 ms（+8.9%），P95 39.86→43.23 ms（+8.5%），CPU/千请求 +8.4%，RSS P95 +177.1 MiB，堆 P95 +43.2 MiB。
3. **GC 情况：** 300 RPS 三类全开 Young GC 中位数 411 次、Full GC 0 次，600 秒内累计停顿 1431 ms，占比 0.2385%，最大单次停顿中位数 26.06 ms。
4. **内存口径必须分开看：** 8 GiB 是云主机系统内存；512 MiB 是 JVM 固定提交堆；RSS 是整个 Java 进程实际常驻内存，包含堆、Metaspace、代码缓存、线程栈、直接内存和 Agent 原生结构。
5. **建议使用 `-Xms512m -Xmx1024m`。** 该配置在当前 300 RPS 三类全开下仍只提交 512 MiB，RSS P95 752.2 MiB，与 512/512 MiB 同量级，同时为突发分配保留按需扩容空间。
6. **不建议当前场景固定 1 GiB。** 三类全开 RSS P95 从 777.8 增至 1095.8 MiB（+318.0 MiB）；GC占比下降，但 P95 三轮范围重叠，无法证明获得稳定业务性能收益。
