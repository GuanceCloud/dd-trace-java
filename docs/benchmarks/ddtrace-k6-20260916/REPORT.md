...
2. **无探针 → 单项探针：开销有明显差异。** 以 300 RPS 为例，仅 Metrics 几乎不改变业务延迟（平均 34.77→34.82 ms，P95 39.86→40.23 ms），但 RSS P95 增加 47.0 MiB；仅 Profile 的 CPU 增加 1.0%、RSS 增加 94.0 MiB；仅 Trace 的 CPU 增加 6.0%、平均延迟增加 1.14 ms、P95 增加 1.86 ms、RSS 增加 70.6 MiB。单项选择应按观测目标，而不是按统一开关处理。
3. **单项探针 → 三类全开：组合开销高于任一单项。** 三类全开相对无探针平均延迟增加 8.9%、P95 增加 8.5%、CPU/千请求增加 8.4%、RSS P95 增加 177.1 MiB；相对仅 Trace 还增加 1.96 ms 平均延迟、1.51 ms P95、2.3% CPU 和 106.5 MiB RSS，说明 Profile 与 Metrics 叠加后需要单独评估资源余量。
